...
```bash
cd Nexify-server/client
npm install
```

```bash
cd Nexify-server/server
npm install
```
...
Moderators have specific email domain (`@mod.nexify.com`). When registering with an email from this domain, the user is automatically assigned the moderator role. Moderators can be assigned to different communities from the admin dashboard.
